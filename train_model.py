...
def generate_dataset(n_samples=5000, seed=42):
    ...
    for _ in range(n_real):
        ...
        account_age = int(np.random.uniform(6, 96))  # months
        ...
        has_pic = np.random.choice([0, 1], p=[0.05, 0.95])
        has_bio = np.random.choice([0, 1], p=[0.10, 0.90])
    ...
    for _ in range(n_fake):
        ...
        if fake_type == 'bot':
            ...
            likes = int(followers * np.random.uniform(0.01, 0.12) * np.random.uniform(0.7, 1.3))
            comments = int(followers * np.random.uniform(0.0001, 0.001))
...
def engineer_features(df):
    ...
    df['completeness_score'] = (
        df['has_profile_pic'] +
        df['has_bio'] +
        (1 - df['has_numeric_username']) * 0.5 +
        df['posts'].apply(lambda x: 1 if x > 5 else 0)
    )
